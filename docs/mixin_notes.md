...
| Mixin 类路径 | 目标类 | 注入点 | 风险/说明 |
|---|---|---|---|
| `mixin/ae/MixinPacketPickBlock.java` | `appeng.core.sync.packets.PacketPickBlock` | `serverPacketData` HEAD，`remap = false`（**fix52 起不再 cancellable**） | fix47：世界里中键取物的兜底。判定顺序刻意做成"能不管就不管"——解析不出物品 / 背包已有 / 无可用无线终端 / 网络还有存量 一律放行原版，仅「无存量 + 有样板」才接管。被点物品经反射读私有 `pickedBlock`，取不到就放行。整个注入体 try/catch 包裹，异常一律退化为放行。<br>**fix52**：判定仍在同步段，**开界面改为 `ServerTerminalHelper.scheduleServerTask` 归队服务端 tick 线程**——AE2 的包处理器走 FML `FMLEventChannel` 的 `ServerCustomPacketEvent`，**运行在网络线程上**，在那种线程里替换 `player.openContainer`／开界面不会生效（NEI 面板中键那条路径一直归队，实测可用；两条路径共用同一个开界面方法，差异只有线程）。同时不再 `cancel()`：目标情形下原版本就无操作。 |
