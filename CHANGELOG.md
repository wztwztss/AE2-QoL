...
1. 覆盖板列表由 `CoverRegistry.getAll()`（全服）改为 `CoverRegistry.getByNetwork(terminal.getNetworkId())`
   —— 这正是该方法注释一直写着的设计意图（"统计终端只显示同网络的覆盖板"），
   此前 GUI 用 `getAll()` 属于**实现与注释不一致**，本次对齐。
   - **终端未绑定网络（邻接直连，`networkId` 为空）时 `getByNetwork("")` 仍返回全部**（该方法既有语义），
     此路径行为不变；
   - 只有"终端已绑网络"时才会隐藏"绑到别的网络"或"未绑定的邻接覆盖板"。
2. 新增 `IntSyncValue`（`sm_terminal_covers_hidden`）把**被过滤掉的数量**同步给客户端：
   列表为空时显示「本终端所连网络下没有覆盖板（另有 N 个属于其他网络，已隐藏）」，
   而不是留一片空白——否则"覆盖板怎么不见了"会变成无法定性的现象（本项目一贯原则）。
3. 扫描逻辑从 `collectCoverRows` 收拢进新的 `CoverScan` 内部类（行列表与隐藏计数共用**一次**扫描，
   维持 20 tick 节流，避免 getter 每 tick 遍历注册表）。发信器侧仍用 `RowCache`。
