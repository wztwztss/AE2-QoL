...
| 49 | 库存缓存永不过期：`NetworkInventoryCache.invalidate()` 定义后**从未被任何代码调用**，终端关闭后缓存不失效 → tooltip/书签角标可能长期显示陈旧的存量与可合成状态。修复：`MixinGuiMEMonitorable` 新增 `onGuiClosed` TAIL 注入（remap=true 解析 SRG 名）调用 `invalidate()`，终端关闭即清缓存 | `client/NetworkInventoryCache.java:68` + `mixin/nei/MixinGuiMEMonitorable.java` | 🟡 | ✅ 已修复（2026-08-23） |
