...
> **工作流程（强制，第一优先）**：任何任务开始前，先加载 skill `ae2qol-workflow`（项目内 `.dsh/skills/ae2qol-workflow/SKILL.md`）。
> 流程骨架：**先反复提问把问题确认到 99%**（此阶段禁止读源码/给方案/改文件）→ 用户口令「确认理解，开始读源码」
> → **只读源码取证**并给出有证据的修复思路 → 用户口令「确认方案，开始修改」→ 实施。
> 不清楚问题不动手、没有证据不给方案、未获确认不改文件。
