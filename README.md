...
适配：GTNH 2.9.0-beta-1（Minecraft 1.7.10）| 当前版本：**3.19.0-fix40** | 作者：wztwzt

---

## 📦 安装

1. 将 `AE2-QoL-3.19.0-fix40.jar` 放入 `.minecraft/mods/`
...
**选择界面**（3.19.0-fix40 重做）：面板分为 标题 / 搜索 / 机器列表 / 操作按钮 / 配方映射 五个区域，互不遮挡；列表每行显示机器图标、名称（含 @D 维度 x,y,z 坐标后缀）与样板槽 空闲/总数，单击选中、双击直接上传、滚轮翻页；下方映射区可把「配方 ID → 目标机器名」写进 ecipe_names.json，也可点「用选中机器」一键填入选中机器名。
