...
**作者：wztwzt · 当前源码版本：3.21.4 · 对照整合包：GTNH 2.9.0-beta-3**

本仓库用于个人存档，暂不开放分发。来源、署名与许可记录见 [CREDITS.md](CREDITS.md)。功能说明不代表所有兼容组合均已通过实机测试。

## 本版变化：3.22.0（智能通配样板）

- **新增物品：智能通配样板**（`Smart Wildcard Pattern`，配方 = 1 张 AE2 空白样板）：**一张样板覆盖一整类配方** ——
  例如「1×任意锭 → 1×对应板」，机器建立样板索引时自动展开成各材质各自**合法**的普通样板，因此所有样板总成
  （GT 2714/2715、PH 22069/22179 与本模组 MK.III 32108、GTNL 21504/21505、AE2 ME 接口）都能识别。
- **从 NEI 一步生成**：右键样板打开配置界面 → 在 NEI 里对着配方**点加号** → 当场推导出规则与配方模板 → 保存（服务端写入）。
  矿辞优先（GT 权威 `OrePrefixes.detectPrefix`），流体占位物品自动跳过，编程电路按 `gt.integrated_circuit` 识别。
- **可视化编辑（四页）**：规则 / 覆盖预览（逐条「排除」即进黑名单）/ 黑名单（支持 `*` `?`）/ 电路（1~24 + 铸模、模头、透镜等不消耗物品）。
  预览与机器用的是**同一个展开器**，不会出现"预览能出、实际出不来"。
- **内置编程电路**：机器读取样板时自动写入其虚拟电路槽（GT 官方 `IConfigurationCircuitSupport` + `GTUtility.getIntegratedCircuit`）；
  优先级 **样板自带 > 槽位 > 整机**；样板没设电路时**不动**机器原有电路。
- **新增配置 `smart_wildcard_expand_cap`**（默认 **512**）：单张样板展开候选上限；超限**截断并记 WARN**（不静默少给）。
  同类模组在服务端网络钩子主路径上无界展开是已知卡服点，本模组刻意加上限。
- 已知边界：展开发生在样板索引期（放/换样板、读档时），一张样板覆盖**一个配方类**；详见指南页「智能通配样板」。

## 本版变化：3.21.4（智能倍增单次推送上限改为可配置）

- **新增配置 `smart_doubling_push_cap`**（默认 **4096**，范围 1~2147483647）：智能倍增**单次（每 tick）推送轮数上限**。
  想让每批更大就调大它（`smart_doubling_max_rounds` 保持 0=不限即可）。默认 4096 是审计项 #51（功率探测
  一次索要超量电量会遍历全部储能设备，O(P)）与 #73（1T 级订单把客户端物品更新淹没）修复时定下的安全值；
  调大的代价是每 tick 提取量与物品更新量线性上升。可在 `settings.json`、游戏内「Mods → AE2 QoL → Config」页、
  或 `/ae2qof status` 查看/修改。
- **修掉一条诊断误报**：手动关闭开关时未清除"期望开启"登记，会让 CPU 侧误报
  「刚要求开启但服务端开关仍为 false」（3.21.3 实测中出现过）。
- 说明：你看到的"几万几万地发"= `4096 轮/次 × 样板每轮产出`；**这是设计上限**，不是故障。
...
2. 在与本项目匹配的 GTNH 环境中，用 `AE2-QoL-3.21.4.jar` 替换旧的 AE2 QoL JAR，不要同时保留多个版本。
