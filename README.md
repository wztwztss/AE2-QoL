...
适配：GTNH 2.9.0-beta-1（Minecraft 1.7.10）| 当前版本：**3.4.0** | 作者：wztwzt

---

## 📦 安装

1. 将 `AE2-QoL-3.4.0.jar` 放入 `.minecraft/mods/`
...
- **3.3.5**：修复 GT/PH 样板输入机勾选智能倍增后「完全无效 + 无法发配物品」——功率门槛误用 N× 总电且不足即跳过介质（无回退），原料探测要求严格全量匹配导致 N 静默降为 1
- **3.3.6**：`smart_doubling_max_rounds` 默认改为 0=不限（GT 仓缓冲无上限可一次全发；PH 仓按缓冲空间自取，ME 接口按相邻机器容量上限发配）；新增游戏内配置页面（Mods → AE2 QoL → Config），OP 修改即时生效并同步全服
- **3.3.7**：性能修复——超大订单（1T 量级）不再卡死（功率钳制改 O(1) 一次查询、等待输出批量记账、防 int 溢出）；修复 PH/GT 舱室提示换行；配置页显示可调范围 + 新增「映射编辑」页（配方名映射 `recipe_names.json`、记住的供应器 `remembered_providers.json` 均可客户端热修改并写盘）

### 14. 样板 + 接口二合一终端（改造原生接口终端）

在**原版 AE2 接口终端**（Interface Terminal）的右侧新增一块**样板编码面板**，把样板编码功能直接并入接口管理界面——**接口样板管理与样板编码同一界面完成**，无需再单独打开样板终端。

- **改造方式**：直接混入原生 `GuiInterfaceTerminal` / `ContainerInterfaceTerminal`，所以**有线接口终端与 ae2fc 无线接口终端自动共用同一面板**，无需任何额外配置
- **面板按钮（4 行）**：`编`（编码当前面板配方）/ `清`（清空面板）/ `×2`（输出倍率加倍）；`合↔处`（合成/处理模式切换，绿色高亮）/ `替`（替代）/ `备`（备份替代）；`N`（从 NEI 当前配方页填入面板）/ `↑`（上传）/ `←`（召回）；`⇄`（交换主/副产物）/ `OV`（NEI 叠加层开关）
- **NEI 联动**：`N` 一键抓取鼠标悬浮的 NEI 配方填入面板，再点 `编` 直接编码；处理配方自动识别
- **上传**：把已编码样板自动上传到网络中的接口/总成（Shift+点击强制打开供应器选择界面）；**召回**取回最后一个匹配样板；**交换**轮换主产物/副产物位置
- **处理模式网格**：按输入物品行数动态扩展（输入行数决定输出/结果/编码槽行位置），面板内也可直接放入物品构成配方
