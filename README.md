...
**作者：wztwzt · 当前源码版本：3.19.0-fix52 · 对照整合包：GTNH 2.9.0-beta-3**

本仓库用于个人存档，暂不开放分发。来源、署名与许可记录见 [CREDITS.md](CREDITS.md)。功能说明不代表所有兼容组合均已通过实机测试。

## 本版变化：fix52

- **修复「世界里中键，网络无存量但有样板时不弹下单页」**：AE2 的包处理器运行在**网络线程**上，
  而在该线程里替换 `player.openContainer` / 打开界面不会生效，因此整条兜底逻辑静默失败。
  NEI 物品面板中键那条路径一直显式归队到服务端 tick 线程（实测可用），两条路径共用同一个开界面方法，
  差异只有线程——现在世界中键也按同样方式归队，并且**不再拦截原版**（目标情形下原版本来就是无操作）。
  同时把存量判定的异常兜底由「当作有存量」改为「当作无存量」并记一条警告，
  避免一次判定异常就让该功能永久静默失效。
  **需要实测确认**：无存量 + 有样板能弹出下单页；有存量仍原样取物；两者都没有仍无反应。
