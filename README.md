...
适配：GTNH 2.9.0-beta-1（Minecraft 1.7.10）| 当前版本：**3.18.0** | 作者：wztwzt

---

## 📦 安装

1. 将 `AE2-QoL-3.18.0.jar` 放入 `.minecraft/mods/`
...
新增 GT 仓室形态的自适应电网系统，通过**数据棒**配置网络 ID 实现多仓室共享：

- **自适应终端**（ID 32100）：右键打开 GUI，通过 3 个标签页管理网络（主状态 / 自适配设置 / 频率设置）
- **自适配输入仓**（ID 32102）：自动匹配终端设置的电压等级和电流
- **自适配激光源仓**（ID 32103）：激光输出仓，自动匹配等级
- **自适配动力仓**（ID 32104）：动力输出仓，自动匹配等级
- **自适配激光靶仓**（ID 32105）：激光接收靶仓，自动匹配等级
- **网络数据棒**：右键终端写入网络配置（Shift+右键终端初始化），右键仓室绑定，右键终端读取
