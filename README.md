...
### 2.5 合成完成产物展示条（标准 ME 终端第一行）

合成 CPU 完成订单后，产物自动显示在**标准 ME 终端第一行**（搜索框下方，类似标记区的临时展示条）：

- 每条保留 **60 秒**后自动消失（最后 5 秒渐隐）；同物品新完成时合并数量并重新计时；满 9 格顶掉最旧
- **点击格子直接提取一组**到背包（提取失败会在聊天栏提示）；悬停显示物品名与数量
- 与右上角「合成完成」横幅并存；仅在标准 ME 终端显示（合成终端 / 接口终端 / 二合一终端 / 无线终端不显示）
- 注意：展示期间该行的网络物品暂不可点击，60 秒后自动恢复
